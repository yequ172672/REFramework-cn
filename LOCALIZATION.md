...
如需手动指定，可在游戏内按 `Insert` 打开菜单，进入 **配置（Configuration） → 语言（Language）**，选择：

- **Auto (System)**（默认，跟随系统语言）
- **简体中文**
- **English**

手动选择后立即生效，并保存到 `re2_fw_config.txt`，下次启动沿用；选择 **Auto (System)** 时，每次启动都会按系统语言重新决定。
