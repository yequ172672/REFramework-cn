...
void Hooks::on_draw_ui() {
    if (!ImGui::CollapsingHeader(REF_TR("Performance"))) {
        return;
    }

    ImGui::Checkbox(REF_TR("Enable Profiling"), &m_profiling_enabled);

    if (!m_profiling_enabled) {
        return;
    }

    ImGui::Text(REF_TR("Application Entry Times"));
    ...
    ImGui::Text(REF_TR("Total REFramework Time: %.3fms"), std::chrono::duration_cast<std::chrono::duration<float, std::milli>>(total_reframework_time).count());
    ImGui::Text(REF_TR("Total Game Time: %.3fms"), std::chrono::duration_cast<std::chrono::duration<float, std::milli>>(total_game_time).count());
    ...
    for (auto name : sorted_times) {
        ...
        if (ImGui::TreeNode(name)) {
            ImGui::Text(REF_TR("Game Time: %s: %.2fms"), name, entry.callback_time.count() / 1000000.0f);
            ImGui::Text(REF_TR("REFramework Pre Time: %.2fms"), entry.reframework_pre_time.count() / 1000000.0f);
            ImGui::Text(REF_TR("REFramework Post Time: %.2fms"), entry.reframework_post_time.count() / 1000000.0f);
            ImGui::Text(REF_TR("Total Time: %.2fms"), (entry.callback_time + entry.reframework_pre_time + entry.reframework_post_time).count() / 1000000.0f);
