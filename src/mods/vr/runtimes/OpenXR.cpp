...
void OpenXR::display_bindings_editor() {
    const auto current_interaction_profile = this->get_current_interaction_profile();
    ImGui::Text(REF_TR("Interaction Profile: %s"), current_interaction_profile.c_str());

    if (ImGui::Button(REF_TR("Restore Default Bindings"))) {
    ...
    if (ImGui::Button(REF_TR("Save Bindings"))) {
        this->save_bindings();
    }
    ...
    auto display_hand = [&](const std::string& name, uint32_t index) {
        if (current_interaction_profile.empty() || current_interaction_profile == "XR_NULL_PATH") {
            ImGui::Text(REF_TR("Interaction profile not loaded, try putting on your headset."));
            return;
        }
        ...
        if (ImGui::TreeNode(name.data())) {
            ...
            ImGui::InputText(REF_TR("New Binding (e.g. /user/hand/left/input/trigger)"), hand.ui.new_path_name, XR_MAX_PATH_LENGTH);
            ImGui::Combo(REF_TR("Action"), &hand.ui.action_combo_index, known_actions_cstr.data(), known_actions_cstr.size());

            if (ImGui::Button(REF_TR("Add Binding"))) {
                XrPath p{};
            ...
            ImGui::Text(REF_TR("Vector2 Associations"));
            for (auto& it : hand.profiles[current_interaction_profile].vector_activators) {
                ...
                for (auto& output : it.second) {
                    ...
                    ImGui::SliderFloat2(REF_TR("Value"), &output.value[0], -1.0f, 1.0f);
                    ImGui::PopID();
                }

                if (ImGui::Button(REF_TR("Insert New Output"))) {
            ...
            ImGui::Combo(REF_TR("New Vector2 Activator"), &hand.ui.activator_combo_index, known_actions_cstr.data(), known_actions_cstr.size());
            ImGui::Combo(REF_TR("New Vector2 Modifier"), &hand.ui.modifier_combo_index, known_vector2_actions_cstr.data(), known_vector2_actions_cstr.size());
            ImGui::Combo(REF_TR("New Vector2 Output"), &hand.ui.output_combo_index, known_actions_cstr.data(), known_actions_cstr.size());
            ImGui::SliderFloat2(REF_TR("New Vector2 Value"), &hand.ui.output_vector2[0], -1.0f, 1.0f);

            if (ImGui::Button(REF_TR("Add Vector2 Association"))) {
